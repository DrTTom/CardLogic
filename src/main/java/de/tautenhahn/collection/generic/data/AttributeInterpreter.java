...
public abstract class AttributeInterpreter
{
    ...
    /**
     * Returns true if value should be added to a search index, thus enabling free text search. Otherwise, the attribute
     * is searchable via its {@link #computeCorrellation(String, String, DescribedObject)} method.
     */
    ...
    /**
     * Returns null if value is OK and message otherwise. System will call this method only if values is present.
     *
     * @param value internal value as present in the object
     * @param context provide a described object as context to allow the following methods changing behavior. For
     *     instance "Vienna Pattern" is legal for French suits but forbidden in German or Spanish suits.
     */
    ...
    /**
     * Same as {@link #computeCorrellation(String, String, DescribedObject)} but never called with null arguments.
     */
    ...
    /**
     * Note that an empty String is filled in instead of a missing value because HTML and JavaScript front ends cannot
     * recognize null values properly and we do not want "undefined" pre-filled into input elements.
     *
     * @param object
     * @param constructor returns the question, expects text and group
     */
    protected <T extends Question> T createQuestion(DescribedObject object, BiFunction<String, String, T> constructor)
    {
    ...
    /**
     * Returns the value to display for representing given internal value. Values may be different whenever internally
     * used values do not look good. Reasons may be that some artificial keys are used internally.
     * <br>
     * By default, internally missing values are represented by empty String because most front ends can not handle null
